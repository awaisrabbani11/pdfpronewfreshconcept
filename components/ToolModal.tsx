         ...
         <>
            {files.length === 0 && output.length === 0 && (
                <div
                    ...
                    className={`flex flex-col items-center justify-center p-12 border-2 border-dashed rounded-lg cursor-pointer transition-colors ${isDragging ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20' : 'border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
                    <UploadIcon />
                ...
                <div className="space-y-3">
                    ...
                    <ul className="space-y-2">{files.map((file, index) => (
                        <li key={index} className="flex items-center justify-between p-2 bg-gray-100 dark:bg-gray-700 rounded-md text-sm">
                            ...
                            <button onClick={() => removeFile(index)} className="text-gray-400 hover:text-indigo-500 font-bold p-1">&times;</button>
                        </li>))}
                    </ul>
            ...
            <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-4 p-4 bg-gray-100 dark:bg-gray-900 rounded-lg">
                {pageThumbnails.map((page, index) => (
                    <div key={page.id} draggable onDragStart={() => dragItem.current = index} onDragEnter={() => dragOverItem.current = index} onDragEnd={handleSort} onDragOver={e => e.preventDefault()}
                        ...
                        <button onClick={() => deletePage(page.id)} className="absolute -top-2 -right-2 bg-indigo-500 text-white rounded-full w-6 h-6 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">&times;</button>
                    </div>
                ))}
            </div>
        </div>
            ...
            <div className={`relative bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-4xl mx-4 transition-transform duration-300 ${isOpen ? 'scale-100' : 'scale-95'}`} onMouseDown={e => e.stopPropagation()}>
                ...
                {output.length === 0 && (
                  <div className="p-6 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-200 dark:border-gray-700 rounded-b-lg">
                      <button onClick={handleProcess} disabled={files.length === 0 || isLoading} className="w-full bg-indigo-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-indigo-700 disabled:bg-gray-400 dark:disabled:bg-gray-600 disabled:cursor-not-allowed transition-colors duration-300">{tool.title}</button>
                  </div>
                )}
            </div>
        </div>
